...
class UploadEntity implements SourceInterface, \JsonSerializable
{
    ...
    #[ORM\OneToOne(mappedBy: 'uploadEntity', targetEntity: UploadResult::class, cascade: ['persist', "merge", "detach"])]
    ...
    public function progressTo(int $resultStatus, array $resultData): void
    {
        ...
        $this->uploadResult->setStatus($resultStatus);
        $this->uploadResult->setProgress((int) $resultData['progress']);
        $this->uploadResult->setVulnerabilitiesFound((int) $resultData['progress']);
        $this->uploadResult->setUnaffectedVulnerabilitiesFound((int) $resultData['unaffectedVulnerabilitiesFound']);
    }
}
